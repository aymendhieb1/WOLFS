...
public class SessionService implements IService<Session> {
    ...
    public void ajouter(Session session) {
        ...
        try {
            ...
            pst.setDate(1, java.sql.Date.valueOf(session.getDate_sess())); // Convert LocalDate to SQL Date
            pst.setTime(2, java.sql.Time.valueOf(session.getTime_sess())); // Convert LocalTime to SQL Time
